...
from xtcsg.common.constants import CompScalerMeans, CompScalerStds
from xtcsg.common.data_utils import StandardScaler, chemical_symbols
from xtcsg.pl_data.dataset import TensorCrystDataset
from xtcsg.pl_data.datamodule import worker_init_fn
...
def get_model_path(eval_model_name):
    import xtcsg
    model_path = (
        Path(xtcsg.__file__).parent / 'prop_models' / eval_model_name)
    return model_path
...
def get_crystals_list(
        frac_coords, atom_types, frac_occupancy, lengths, angles, num_atoms, low_atomic_numbers):
    """
    args:
    ...
    """
    assert frac_coords.size(0) == atom_types.size(0) == num_atoms.sum() == frac_occupancy.size(0) == low_atomic_numbers.size(0)
    ...
    for batch_idx, num_atom in enumerate(num_atoms.tolist()):
        ...
        cur_frac_occupancy = frac_occupancy.narrow(0, start_idx, num_atom)
        cur_atom_types = atom_types.narrow(0, start_idx, num_atom)
        cur_lengths = lengths[batch_idx]
        cur_angles = angles[batch_idx]
        cur_low_atomic_numbers = low_atomic_numbers.narrow(0, start_idx, num_atom)

        crystal_array_list.append({
            'frac_coords': cur_frac_coords.detach().cpu().numpy(),
            'atom_types': cur_atom_types.detach().cpu().numpy(),
            'frac_occupancy': cur_frac_occupancy.detach().cpu().numpy(),
            'lengths': cur_lengths.detach().cpu().numpy(),
            'angles': cur_angles.detach().cpu().numpy(),
            'low_atomic_numbers': cur_low_atomic_numbers.detach().cpu().numpy(),
        })
