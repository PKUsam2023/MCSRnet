...
def build_ele_list(crystal_str):
    crystal = Structure.from_str(crystal_str, fmt='cif')

    high_occupancy_elements = []
    low_occupancy_elements = []
    high_occupancy_ratios = []
    for site in crystal:
        ...
        if len(species_and_occu) == 1:
            ...
            high_occupancy_ratios.append(occupancy)
            if occupancy == 1:  
                high_occupancy_elements.append(element.symbol)
                low_occupancy_elements.append(element.symbol)   
            elif occupancy < 1:
                high_occupancy_elements.append(element.symbol)
                low_occupancy_elements.append("None")   
            ...
            element2, occ2 = species_and_occu_sorted[1]
            high_occupancy_elements.append(element1.symbol)  
            high_occupancy_ratios.append(occ1)
            low_occupancy_elements.append(element2.symbol)

    return high_occupancy_elements, high_occupancy_ratios, low_occupancy_elements
...
def build_crystal(crystal_str, niggli=True, primitive=False):
    ...
    high_occupancy_elements, high_occupancy_ratios, low_occupancy_elements = build_ele_list(crystal_str)

    canonical_crystal = Structure(
        lattice=Lattice.from_parameters(*crystal.lattice.parameters),
        species=high_occupancy_elements,
        coords=crystal.frac_coords,
        coords_are_cartesian=False,
    )
    # match is gaurantteed because cif only uses lattice params & frac_coords
    # assert canonical_crystal.matches(crystal)
    return canonical_crystal, high_occupancy_ratios, low_occupancy_elements
...
def process_one(row, niggli, primitive, graph_method, prop_list, use_space_group = False, tol=0.01):
    crystal_str = row['cif']
    crystal , high_occupancy_ratios, low_occupancy_elements = build_crystal(
    ...
    low_atomic_numbers = [element_to_atomic_number[element] for element in low_occupancy_elements]
    result_dict.update({
        #'mp_id': row['material_id'],
        'cif_name': row['filename'],
        "low_atomic_numbers": low_atomic_numbers,
        "high_occupancy_ratios": high_occupancy_ratios,
        'cif': crystal_str,
        'xrd_embeddings': row['xrd_embeddings'],
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    mpid_to_results = {result['cif_name']: result for result in unordered_results}
    ordered_results = [mpid_to_results[df.iloc[idx]['filename']]
