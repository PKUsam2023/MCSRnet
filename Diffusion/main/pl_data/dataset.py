...
class CrystDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        values = data_dict['xrd_embeddings']
        values = values.strip('[]')
        xrd_embeddings = np.fromstring(values, sep=' ')
        low_atomic_numbers = data_dict["low_atomic_numbers"]
        frac_occupancy = torch.Tensor(data_dict['high_occupancy_ratios'])
        ...
        data = Data(
            ...
            low_atomic_numbers = torch.LongTensor(low_atomic_numbers),
            ...
            y=prop.view(1, -1),
            xrd_embeddings = torch.Tensor(xrd_embeddings), 
        )
...
def main(cfg: omegaconf.DictConfig):
    ...
    from xtcsg.common.data_utils import get_scaler_from_data_list
